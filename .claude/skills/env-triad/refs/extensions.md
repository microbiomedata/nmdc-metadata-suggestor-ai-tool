...
Seeds live in `EXPANSION_SEEDS` in `src/nmdc_metadata_suggestor_ai_tool/envo.py`, and `envo.format_expansion_context(interface, slot)` prints them with definitions. Every CURIE there is checked by `tests/test_envo.py` to exist, to be current, and to sit under its slot's anchor. So is every CURIE written into these reference files — see `tests/test_env_triad_references.py`.
