...
class MetadataFieldSuggestion(BaseModel):
    ...
    id: str | None = Field(None, description="Unique identifier if the sample record is associated with a specific input")
    ...
    value: str | list | dict | int | float = Field(
        default="",
        description="The recommended value for the metadata field"
    )

class LLMOutput(BaseModel):
    """Top-level output payload returned by the LLM."""

    metadata_fields: list[MetadataFieldSuggestion] = Field(default_factory=list, description="List of metadata field suggestions")
