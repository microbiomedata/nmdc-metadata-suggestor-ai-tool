...
Usage::
    ...
    make get-abstracts
    make get-abstracts-from-file FILE=dois.tsv OUT=results/
"""
...
def _read_dois_from_file(file_path: Path) -> list[str]:
    ...
    if "doi" in first_line.lower():
        ...
        if "doi_value" in fieldnames:
            ...
            sys.exit(1)
        # Map lowered fieldnames back to actual fieldnames for lookup
...
def cmd_get_abstracts_from_file(
    ...
    *all_categories* to ``False`` to restore the publication-only gate.
    ...
    # Write combined results as a single JSON array
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print(
            ...
            "|get-abstract|get-abstracts|get-abstracts-from-file> [DOI|FILE] [OUT]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if command == "validate":
        ...
        for arg in sys.argv[3:]:
            if arg.startswith("sources="):
                ...
                out_dir = arg
        cmd_get_abstracts_from_file(sys.argv[2], out_dir, sources=sources)
    else:
